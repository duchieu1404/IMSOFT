...
router.post('/user_sync_data2', async function (req, res, next) { // backup
    const startTime = Date.now(); 
	try{  
		await req.app.UserDA.user_sync_data(req.body, function (err, data) {
            ...
            if (err || !data) {
            console.error("user_sync_data error=>",JSON.stringify(req.body),"errorinfo=>",e.toString(),"time==>", endTime - startTime);
                return res.json({
                    status: 1,
                    msg: "ServerMsg/api_fail"
                });
